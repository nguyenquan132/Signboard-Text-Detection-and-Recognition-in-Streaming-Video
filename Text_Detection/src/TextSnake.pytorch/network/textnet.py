...
class TextNet(nn.Module):
    ...
    # I adjusted this function versus author by adding device variable, this allows to use both CPU and GPU
    def load_model(self, model_path, device):
        print('Loading from {}'.format(model_path))
        # state_dict = torch.load(model_path) => Author
        if device == 'cpu': state_dict = torch.load(model_path, map_location=torch.device(device)) # added code by me
        else: state_dict = torch.load(model_path) # added code by me
