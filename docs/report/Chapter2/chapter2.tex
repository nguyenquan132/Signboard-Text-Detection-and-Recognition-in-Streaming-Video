...
\begin{figure}[t]
    ...
    \caption{Hình ảnh minh họa tổng quan quy trình phát hiện đối tượng. Ảnh đầu vào được xử lý qua mạng nơ-ron để trích xuất đặc trưng và dự đoán vị trí (hộp giới hạn) cùng phân loại các đối tượng (nhãn lớp) xuất hiện trong khung hình, tham khảo từ Analytics Vidhya~\cite{analyticsvidhya_yolo}.}
...
\begin{figure}[t]
    ...
    \caption{Hình ảnh minh họa quá trình nhận dạng văn bản ngoại cảnh đầu-cuối (End-to-End Scene Text Recognition), tham khảo từ \cite{liu2018scene}.}
    \label{fig:chapter2_end_to_end_STR}
\end{figure}
